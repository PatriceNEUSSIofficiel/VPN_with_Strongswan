...
docker run -tid --privileged  --network ubuntu_net --name ubuntu1-container -v /var/run/docker.sock:/var/run/docker.sock ubuntu1-image sleep infinity
...
docker run -tid --privileged --network ubuntu_net2 --name ubuntu2-container -v /var/run/docker.sock:/var/run/docker.sock ubuntu2-image sleep infinity
...
echo "centos-container(1,2,3,4,5,6) ips : "
